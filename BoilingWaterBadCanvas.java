...
public class BoilingWaterCanvas extends Canvas {
    ...
    double dt = 1E-2;
    double collision_unsticker_dt = 1E-2;
    ...
    boolean[][] moleculemoleculejustcollided = new boolean[N][N];
    boolean[] moleculetopwalljustcollided = new boolean[N];
    boolean[] moleculebottomwalljustcollided = new boolean[N];
    boolean[] moleculeleftwalljustcollided = new boolean[N];
    boolean[] moleculerightwalljustcollided = new boolean[N];
    ...
    public void moleculeToWallCollisions() {
        ...
        for (int i=0; i<N; i++) {
            ...
            if (moleculex[i] - moleculeradius <= xmin && !moleculeleftwalljustcollided[i]) { 
                moleculevx[i] = Math.abs(moleculevx[i]);
                System.out.println("collision with left boundary, molecule " + i + "vx = " + moleculevx[i] + " reversed");
                moleculeleftwalljustcollided[i] = true;
            } else if (moleculex[i] - moleculeradius > xmin && moleculeleftwalljustcollided[i]) {
                moleculeleftwalljustcollided[i] = false;
            }

            if (moleculex[i] + moleculeradius >= xmax && !moleculerightwalljustcollided[i]) {
                moleculevx[i] = -Math.abs(moleculevx[i]);
                System.out.println("collision with right boundary, molecule " + i + "vx = " + moleculevx[i] + " reversed");
                moleculerightwalljustcollided[i] = true;
            } else if (moleculex[i] + moleculeradius < xmax && moleculerightwalljustcollided[i]) {
                moleculerightwalljustcollided[i] = false;
            }

            if (moleculey[i] - moleculeradius <= ymin && !moleculebottomwalljustcollided[i]) {
                moleculevy[i] = Math.abs(moleculevy[i]);
                System.out.println("collision with bottom boundary, molecule " + i + "vy = " + moleculevy[i] + " reversed");
                moleculebottomwalljustcollided[i] = true;
            } else if (moleculey[i] - moleculeradius > ymin && moleculebottomwalljustcollided[i]) {
                moleculebottomwalljustcollided[i] = false;
            }

            if (moleculey[i] + moleculeradius >= ymax && !moleculetopwalljustcollided[i]) {
                moleculevy[i] = -Math.abs(moleculevy[i]);
                System.out.println("collision with top boundary wall, molecule " + i + "vy = " + moleculevy[i] + " reversed");
                moleculetopwalljustcollided[i] = true;
            } else if (moleculey[i] + moleculeradius < ymax && moleculetopwalljustcollided[i]) {
                moleculetopwalljustcollided[i] = false;
            }
                ...
                if (i != j) {
                    if (dist(i,j) <= 2.0 * moleculeradius) {
                        ...
                        while (dist(i,j) <= 2.0 * moleculeradius) {
                            System.out.println("unsticking a collision");
                            ...
                            if (newyi - moleculeradius > ymin && newyi + moleculeradius < ymax) { 
                                moleculey[i] = newyi;
                            }
                            if (newxj - moleculeradius > xmin && newxj + moleculeradius < xmax) {
                                moleculex[j] = newxj;
                            }
                            if (newyj - moleculeradius > ymin && newyj + moleculeradius < ymax) {
                                moleculey[j] = newyj;
                            }
                            */
                        }
